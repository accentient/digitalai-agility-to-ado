...
Describe "WriteTrackingTags: the tool's own tags are off unless the config says otherwise" {
  ...
  It "ships the switch in every config file present, defaulted off" {
    # Each config file is checked whenever it is there. mappings.json is gitignored, so a fresh
    # clone has only the sample; mappings.sample.json is the public tool's template and lives with
    # that repo, so this one has only the live file. The count below is what keeps this from
    # passing vacuously if both ever go missing.
    $checked = 0
    ...
    $checked | Should -BeGreaterThan 0 -Because "at least one config file must be present to check"
  }
}
...
Describe "WriteAgilityHyperlink: a link back to the Agility source record, off unless the config says otherwise" {
  ...
  It "builds the assetdetail.v1 url from the configured base and the Number, with no doubled slash" {
    BuildAgilityUrl 'S-40245'   | Should -Be 'https://www7.v1host.com/YourInstance/assetdetail.v1?Number=S-40245'
  ...
  It "builds one Hyperlink relation op, with the Number as the comment the Links tab shows" {
    ...
    $op.value.url | Should -Be 'https://www7.v1host.com/YourInstance/assetdetail.v1?Number=S-40245'
  ...
  It "builds nothing when the item's own Agility Links already hold the same url, whatever its case" {
    $item = [pscustomobject]@{ Number = "S-40245"; LinkUrls = @("https://WWW7.v1host.com/yourinstance/assetdetail.v1?number=s-40245") }
  ...
  Context "in the create" {
    ...
    It "rides in the create beside the item's own Agility Links when the switch is on" {
      ...
      @($links | ForEach-Object { $_.value.url }) | Should -Contain 'https://www7.v1host.com/YourInstance/assetdetail.v1?Number=S-40245'
  ...
  It "ships the switch in every config file present, defaulted off" {
    ...
    # The shipped template must be off. The live file is deliberately not pinned: turning the
    # switch on there is the supported way to use the feature.
    $sample = Join-Path $PSScriptRoot ".." 'mappings.sample.json'
    if (Test-Path $sample)
    {
      (Get-Content $sample -Raw | ConvertFrom-Json).WriteAgilityHyperlink | Should -BeFalse
    }
  }
}
...
Describe "Area path creation" {
  ...
  Context "EnsureAreaPath" {
    ...
    It "posts a top level node against the areas root, with no trailing parent" {
      ...
      EnsureAreaPath ".legacy conversion" $have

      $script:posted[0].Name | Should -Be ".legacy conversion"
    ...
    It "records what it created, so a leaf sees a parent made moments earlier" {
      ...
      EnsureAreaPath ".legacy conversion" $have
      EnsureAreaPath ".legacy conversion\Apps" $have

      $have.ContainsKey(".legacy conversion") | Should -BeTrue
...
Describe "The run set covers every type before anything migrates" {
  ...
  BeforeAll {
    $script:config = [pscustomobject]@{
      Agility = [pscustomobject]@{ BaseUrl = "https://v1host.example/YourInstance" }
    }
  }
...
Describe "Epic extras the audit found: parser" {
  ...
  It "reads the person who closed the Epic" {
    $parsed = ConvertFromAgilityAssets (EpicPayload @{
      'ClosedBy.Name'  = Attr "Alex Rivera"
      'ClosedBy.Email' = Attr "arivera@example.com"
    }) 'Epic'

    $parsed[0].ClosedByName  | Should -Be "Alex Rivera"
    $parsed[0].ClosedByEmail | Should -Be "arivera@example.com"
  }
...
Describe "Epic links and closer reach Azure DevOps" {
  ...
  # 406 of the 773 closers are not org members (Morgan West alone closed 311). Writing one would
...
Describe "BuildCommentBody" {
  ...
  It "escapes HTML so plain text survives intact" {
    $c = [pscustomobject]@{
      AuthorName = "Jordan Blake"; AuthoredAt = "2019-03-04T16:12:00"
  ...
  It "carries an attribution header with the Agility author and date" {
    $c = [pscustomobject]@{
      AuthorName = "Morgan West"; AuthoredAt = "2019-03-04T16:12:00"
      Content = "Input please."; InReplyToAuthor = $null; InReplyToAt = $null
    }

    BuildCommentBody $c | Should -BeLike "*<b>Morgan West</b> wrote in Agility on 4 March 2019:*"
  }

  It "adds a reply marker naming the parent author and date" {
    $c = [pscustomobject]@{
      AuthorName = "Riley Chen"; AuthoredAt = "2019-03-05T14:40:00"
      Content = "Test the credit card case."
      InReplyToAuthor = "Morgan West"; InReplyToAt = "2019-03-04T16:12:00"
    }

    BuildCommentBody $c | Should -BeLike "*in reply to Morgan West (4 March 2019):*"
  }
...
Describe "BuildCommentPatch" {
  ...
  BeforeAll {
    $script:plainComment = [pscustomobject]@{
      AuthorName = "Jordan Blake"; AuthorEmail = "jordan.blake@example.com"
  ...
  It "prefers the author email, because that is what ADO resolves an identity from" {
    ResolveCommentAuthor $script:plainComment | Should -Be "jordan.blake@example.com"
  }
...
Describe "GetAgilityDiscussions" {
  ...
  BeforeAll {
                           ...
                           [string]$content, [string]$replyTo, [string[]]$mentions)
    {
      ...
      Add-Member -InputObject $attributes -NotePropertyName 'Author.Email' -NotePropertyValue ([pscustomobject]@{ value = "$author@example.com" })
  ...
  BeforeEach {
    $script:config = [pscustomobject]@{ Agility = [pscustomobject]@{ BaseUrl = "https://example.invalid/YourInstance" } }
  ...
  It "gives a work item the WHOLE thread, including replies that carry no mention" {
    Mock InvokeAgilityGet {
      [pscustomobject]@{ Assets = @(
        (NewExpression "Expression:1" "Conversation:9" "Jordan"    "2019-03-04T16:12:00" "Question" $null       @("Story:12345"))
  ...
  It "resolves the reply's parent author and date, for the reply marker" {
    ...
    $reply.InReplyToAuthor | Should -Be "Jordan"
  ...
  It "sorts the thread by AuthoredAt, whatever order the wire returned it in" {
    Mock InvokeAgilityGet {
      [pscustomobject]@{ Assets = @(
        ...
        (NewExpression "Expression:1" "Conversation:9" "Jordan"    "2019-03-04T16:12:00" "first"  $null @("Story:12345"))
      ) }
    }
  ...
  It "posts a thread to every work item its conversation mentions" {
    Mock InvokeAgilityGet {
      [pscustomobject]@{ Assets = @(
        (NewExpression "Expression:1" "Conversation:9" "Jordan" "2019-03-04T16:12:00" "Shared" $null @("Story:12345", "Defect:777"))
      ) }
    }
  ...
  It "normalizes a mention oid that carries a moment suffix" {
    Mock InvokeAgilityGet {
      [pscustomobject]@{ Assets = @(
        (NewExpression "Expression:1" "Conversation:9" "Jordan" "2019-03-04T16:12:00" "Hi" $null @("Story:12345:6789"))
      ) }
    }
  ...
  It "resolves the reply parent even when the expression id carries a moment suffix" {
    Mock InvokeAgilityGet {
      [pscustomobject]@{ Assets = @(
        (NewExpression "Expression:1:55" "Conversation:9" "Jordan"    "2019-03-04T16:12:00" "Question" $null          @("Story:12345"))
        (NewExpression "Expression:2"    "Conversation:9" "Shannon" "2019-03-05T14:40:00" "Answer"   "Expression:1" @())
      ) }
    }
...
Describe "AddAdoDiscussion" {
  ...
  BeforeAll {
    ...
    $script:twoComments = @(
      [pscustomobject]@{ AuthorName = "Jordan"; AuthorEmail = "jordan@example.com"; AuthoredAt = "2019-03-04T16:12:00"; Content = "one"; InReplyToAuthor = $null; InReplyToAt = $null }
      [pscustomobject]@{ AuthorName = "Ann";  AuthorEmail = "ann@example.com";  AuthoredAt = "2019-03-05T16:12:00"; Content = "two"; InReplyToAuthor = $null; InReplyToAt = $null }
    )
  }
