...
Describe "MaterializeOwners: add recoverable owners to the org as free Stakeholders" {
  ...
  Context "GetDistinctOwners" {
    ...
    It "dedups an owner across items by email, counts them, and skips an email-less owner" {
      # One page of two assets, then an empty page to stop. Jordan appears twice; Vendor has no email.
      $script:page = 0
      Mock InvokeAgilityGet {
        $script:page++
        if ($script:page -eq 1)
      ...
      @($owners).Count | Should -Be 1 -Because "Jordan is one person and Vendor has no email to add by"
...
Describe "GetAgilityDiscussions" {
  ...
  It "gives a work item the WHOLE thread, including replies that carry no mention" {
    Mock InvokeAgilityGet {
      [pscustomobject]@{ Assets = @(
        ...
        (NewExpression "Expression:2" "Conversation:9" "Riley" "2019-03-05T14:40:00" "Answer"   "Expression:1" @("Member:1013"))
      ) }
    }
  ...
  It "resolves the reply's parent author and date, for the reply marker" {
    Mock InvokeAgilityGet {
      [pscustomobject]@{ Assets = @(
        (NewExpression "Expression:1" "Conversation:9" "Jordan"    "2019-03-04T16:12:00" "Question" $null          @("Story:12345"))
        (NewExpression "Expression:2" "Conversation:9" "Riley" "2019-03-05T14:40:00" "Answer"   "Expression:1" @())
      ) }
    }
  ...
  It "sorts the thread by AuthoredAt, whatever order the wire returned it in" {
    Mock InvokeAgilityGet {
      [pscustomobject]@{ Assets = @(
        (NewExpression "Expression:2" "Conversation:9" "Riley" "2020-05-05T18:30:00" "second" $null @())
  ...
  It "merges every conversation that mentions the same work item, in date order across both" {
    Mock InvokeAgilityGet {
      [pscustomobject]@{ Assets = @(
        (NewExpression "Expression:1" "Conversation:9"  "Jordan"    "2019-03-04T16:12:00" "first"  $null @("Story:12345"))
        (NewExpression "Expression:2" "Conversation:10" "Riley" "2020-05-05T18:30:00" "second" $null @("Story:12345"))
      ) }
    }
