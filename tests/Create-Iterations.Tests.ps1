...
Describe "Reading the timeboxes the migration will write" {
  ...
  BeforeAll {
    $script:config = [pscustomobject]@{
      Agility = [pscustomobject]@{
        BaseUrl = "https://www10.v1host.com/CWI"
        Scopes  = @([pscustomobject]@{ Scope = "Scope:1547"; AreaPath = "Operations" },
                    [pscustomobject]@{ Scope = "Scope:2463"; AreaPath = "User Services" })
      }
  ...
  It "asks Agility for every configured scope and every timeboxed type, excluding Dead items" {
    ...
    @($script:urls | Where-Object { $_ -match ([regex]::Escape([uri]::EscapeDataString("Scope='Scope:2463'"))) }).Count | Should -Be 3
  }
...
Describe "Idempotence: a rerun creates only what is missing" {
  ...
  BeforeAll {
    $script:testConfig = [pscustomobject]@{
      Agility     = [pscustomobject]@{ BaseUrl = "https://www10.v1host.com/CWI"; Scopes = @([pscustomobject]@{ Scope = "Scope:1547"; AreaPath = "Operations" }) }
      AzureDevOps = [pscustomobject]@{ OrganizationUrl = "https://dev.azure.com/contoso"; Project = "IT" }
    }
