...
On the client instance the UI calls an `Issue` a **"Challenge"** and calls a `Defect` an **"Issue"**. So
...
`Scope:4321` and `Scope:5678`, are descendants holding no open Epics, so nothing is lost by
omitting them.
...
Caveats for that phase: `Scope:3456` themes only 17 of 112 open Stories, and 98 open Stories across
...
configured. That scope **is** configured now (`Scope:5678`), so those particular items resolve when
